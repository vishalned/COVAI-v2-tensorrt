...
class DICOM_interface(Abstract_IO):
    ...
    def load_image(self, index):
        ...
        dicom_images_path = self.data_directory
        ...
        dicom_images_path += '/'+os.listdir(dicom_images_path)[0]
        print(dicom_images_path, " this is the path of dcms")
    ...
    def save_prediction(self, sample, output_path):
        ...
        # a = np.zeros((3, 3))
        # np.fill_diagonal(a, sample.get_extended_data()["spacing"])


        # # print(sample.get_extended_data()["spacing"])
        # temp = nib.affines.from_matvec(a)
        # nifti = nib.Nifti1Image(sample.pred_data, temp)
        # # nifti = nib.Nifti1Image(sample.pred_data, sample.get_extended_data()["spacing"])
        # #nifti.get_data_dtype() == pred.dtype
        # # Save segmentation to disk
        # print(sample.pred_data.dtype)
        pred = sample.pred_data.astype(np.uint16)
        sitk_img = sitk.GetImageFromArray(pred)
        pred_file = "output" + ".nii.gz"
        sitk.WriteImage(sitk_img, os.path.join(output_path, pred_file))
